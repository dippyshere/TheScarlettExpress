...
namespace CodingJar.MultiScene.Editor
{
	...
	/// </summary>
	static class AmsCrossSceneReferenceProcessor
    {
	    /// <summary>
        ...
        static List<KeyValuePair<SerializedProperty, Object>> _referenceMap;
        ...
        static readonly Dictionary<Type, CustomDataProcessorDelegate> _customCrossSceneReferenceDataProcessors = new();

        /// <summary>
        ///     Add a specialized custom data resolver to the cross-scene reference system.  This allows specialized classes like
        ///     PlayableDirector (which uses IExposedPropertyTable) to store
        ///     restorable cross-scene reference data.
        /// </summary>
        /// <typeparam name="ObjectType">
        ///     The type that needs custom data to properly store a cross-scene reference (e.g.
        ///     PlayableDirector)
        /// </typeparam>
        /// <param name="customDataProcessor">
        ///     The delegate to compute the data we need to store in order to restore the reference
        ///     properly
        /// </param>
        public static void AddCustomCrossSceneDataProcessor<ObjectType>(CustomDataProcessorDelegate customDataProcessor)
        ...
        static List<EditorCrossSceneReference> ComputeAllCrossSceneReferences()
        {
            UpdateReferencesMap();
        ...
        public static List<EditorCrossSceneReference> GetCrossSceneReferencesForScenes(IEnumerable<Scene> scenes)
        {
            ...
            List<EditorCrossSceneReference> crossSubSceneRefs = ComputeAllCrossSceneReferences();
        ...
        /// </summary>
        static void UpdateReferencesMap()
        {
            ...
            IEnumerable<Object> allMonoBehaviourObjs = Resources.FindObjectsOfTypeAll<MonoBehaviour>()
                .Where(x => !EditorUtility.IsPersistent(x));
            IEnumerable<Object> allScriptableObjs = Resources.FindObjectsOfTypeAll<ScriptableObject>().Where(x =>
            ...
            _referenceMap =
                new List<KeyValuePair<SerializedProperty, Object>>(allMonoBehaviourObjs.Count() +
                                                                   allScriptableObjs.Count());
            ...
            foreach (Type customType in _customCrossSceneReferenceDataProcessors.Keys)
            {
                ...
                IEnumerable<Object> sceneCustomObjects = Resources.FindObjectsOfTypeAll(customType)
                    .Where(x => !EditorUtility.IsPersistent(x));
                PopulateReferenceMap(_referenceMap, sceneCustomObjects);
            }

            AmsDebug.LogPerf(null, "Cross-Scene Reference Map Update: {0}",
        ...
        static void PopulateReferenceMap(List<KeyValuePair<SerializedProperty, Object>> map,
            IEnumerable<Object> allObjects)
        {
            Assembly unityEditorAssembly = typeof(EditorApplication).Assembly;
            Assembly userEditorAssembly = typeof(AmsCrossSceneReferenceProcessor).Assembly;
        ...
        static List<EditorCrossSceneReference> ComputeCrossSceneReferences(
            List<KeyValuePair<SerializedProperty, Object>> map)
        {
            List<EditorCrossSceneReference> refSubSceneMap = new();
            Dictionary<Object, Scene> cache = new();

            // We figure out what the subScene mapping is (from which subScene to which subScene).
            foreach (KeyValuePair<SerializedProperty, Object> pair in map)
            {
                Object fromObject = pair.Key.serializedObject.targetObject;
                Object toObject = pair.Value;
                ...
                EditorCrossSceneReference crossRef =
                    CreateEditorCrossSceneReference(fromObject, fromScene, pair.Key, toObject, toScene);
        ...
        static EditorCrossSceneReference CreateEditorCrossSceneReference(Object fromObject, Scene fromScene,
            SerializedProperty fromProperty, Object toObject, Scene toScene)
        {
            EditorCrossSceneReference crossRef = new();
            ...
            {
                List<GenericData> customData = customDataProcessor(crossRef);
        ...
        static Scene FindSceneCached(Object instance, Dictionary<Object, Scene> cache)
        {
            // Null SubScene
            if (!instance)
            {
            ...
            {
                GameObject gameObj = instance.EditorGetGameObjectFromComponent();
                if (gameObj)
                {
                    scene = gameObj.scene;
                }
        ...
        /// <summary>
        ///     Save all of the passed-in cross-scene references.  The entries in the passed-in list will be removed as they are
        ...
        public static void SaveCrossSceneReferences(List<EditorCrossSceneReference> editorCrossSceneRefs)
        {
            ...
            {
                EditorCrossSceneReference xRef = editorCrossSceneRefs[i];
                ...
                try
                {
                    ...
                    {
                        // Save the object
                        Object initialObject = xRef.fromProperty.objectReferenceValue;
                    ...
                    {
                        AmsDebug.LogError(xRef.fromObject,
                            "Could not perform a runtime resolve on cross-scene reference {0}.\nReason: {1}. Please review Documentation.",
                            serializedReference, ex.Message);
                        continue;
                    }

                    // Record the cross-scene reference
                    AmsCrossSceneReferences crossSceneRefBehaviour =
                        AmsCrossSceneReferences.GetSceneSingleton(xRef.fromScene, true);
                    crossSceneRefBehaviour.AddReference(serializedReference);

                    // Add an updated reference map value
                    if (_referenceMap != null)
                    {
                        _referenceMap.Add(new KeyValuePair<SerializedProperty, Object>(xRef.fromProperty,
                            xRef.fromProperty.objectReferenceValue));
                    }
                }
