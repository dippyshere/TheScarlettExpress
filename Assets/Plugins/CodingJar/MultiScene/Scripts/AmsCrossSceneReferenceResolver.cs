...
namespace CodingJar.MultiScene
{
    ...
    {
	    /// <summary>
	    ///     The method signature for a custom resolver.  A custom resolver can resolve complex cases for cross-scene
	    ///     references.
	    /// </summary>
        ...
        static readonly List<ResolveCrossSceneReferenceDelegate> _resolvers = new();
        ...
        {
            Object fromObject = xRef.fromObject;
            if (!fromObject)
            {
            ...
            if (!toObject)
            {
                throw new ResolveException(string.Format("Cross-Scene Ref: {0}. Could not Resolve toObject {1}", xRef,
                    toObject));
            }
        ...
        /// </summary>
        static bool DefaultResolve(RuntimeCrossSceneReference xRef)
        {
        ...
        static void ResolveToField(object fromObject, Object toObject, string fromFieldPath,
            RuntimeCrossSceneReference debugThis)
        {
                ...
                try
                {
                    ...
                    if (fromObject == null)
                    {
                        throw new ResolveException(string.Format(
                            "Cross-Scene Ref: {0}. Could not follow path {1} because {2} was null", debugThis,
                            fromFieldPath, splitPaths[i]));
                    }
#if !NETFX_CORE
...
#endif
                    {
                        throw new ResolveException(string.Format(
                            ...
                            debugThis, fromFieldPath, splitPaths[i]));
                    }
                }
                catch (Exception ex)
                {
            ...
            {
                throw new ResolveException(string.Format(
                    "Cross-Scene Ref: {0}. Could not parse piece of path {1} from {2}", debugThis, fieldName,
                    fromFieldPath));
            }
        ...
        static bool GetFieldFromObject(object fromObject, string fromField, out FieldInfo field,
            out PropertyInfo property, out int arrayIndex)
        {
            arrayIndex = -1;
            field = null;
            property = null;
        ...
        /// <returns>The object referred to by fromObject.fromField</returns>
        static object GetObjectFromField(object fromObject, string fromField)
        {
            int arrayIndex;
            FieldInfo field;
            PropertyInfo property;
            ...
            Type fieldType = field != null ? field.FieldType : property.PropertyType;

            bool isArray = arrayIndex >= 0;
            if (isArray)
            {
                IList list =
                    (field != null ? field.GetValue(fromObject) : property.GetValue(fromObject, null)) as IList;
                if (list == null)
                {
                    throw new ResolveException(string.Format(
                        "Expected collection of elements for property {0} but field type is {1}", fieldName,
                        fieldType.Name));
                }
                ...
                {
                    throw new ResolveException(string.Format(
                        "Expected collection of at least {0} elements from property {1}", arrayIndex + 1, fieldName));
                }
        ...
        static void AssignField(object fromObject, Object toObject, FieldInfo field, PropertyInfo property,
            int arrayIndex)
        {
            string fieldName = field != null ? field.Name : property.Name;
            Type fieldType = field != null ? field.FieldType : property.PropertyType;
            ...
            if (isArray)
            {
                object listObj = field != null ? field.GetValue(fromObject) : property.GetValue(fromObject, null);
                ...
                catch (Exception ex)
                {
                    ...
                    throw new ResolveException(string.Format(
                        ...
                        fromObject, fieldName, arrayIndex, ex));
                }
...
#endif
            {
                throw new ResolveException(string.Format("Field {0} of type {1} is not compatible with {2} of type {3}",
                    fieldName, fieldType, toObject, toObject.GetType().Name));
            }
...
#if UNITY_EDITOR
        public static void EditorOnly_ResolveToField(object fromObject, Object toObject, string fromFieldPath,
            RuntimeCrossSceneReference debugThis)
        {
