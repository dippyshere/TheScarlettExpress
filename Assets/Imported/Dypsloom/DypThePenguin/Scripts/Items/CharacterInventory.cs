...
namespace Dypsloom.DypThePenguin.Scripts.Items
{
    ...
    public class CharacterInventory : Inventory, IItemUser
    {
        ...
        void Awake()
        {
            m_Character = GetComponent<Character.Character>();
        }
        ...
        public void TickUse(IUsableItem usableItem)
        {
            for (int i = 0; i < usableItem.Actions.Length; i++)
            {
                ...
                {
                    if (usableItem.Actions[i].TargetAction.CanUse == false)
                    {
                        continue;
                    }
        ...
        {
            bool unequipCondition = itemAmount.Item.ItemDefinition.Unique
        ...
        public void Equip(IItem item)
        {
            ...
            if ((Item)item == m_EquippedRightHandItem)
            {
                return;
            }
            ...
            if (item.ItemDefinition.Unique == false
                ...
                && item.ItemDefinition == m_EquippedRightHandItem.ItemDefinition)
            {
                return;
            }
        ...
        public void Unequip(Item item)
        {
            ...
            if (m_EquippedRightHandItem == null || item != m_EquippedRightHandItem)
            {
                return;
            }
