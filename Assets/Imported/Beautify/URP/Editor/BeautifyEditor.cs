...
namespace Beautify.Universal
{
    ...
    public class BeautifyEditor : VolumeComponentEditor
    {
        ...
        {
            public readonly Dictionary<Beautify.SettingsGroup, List<MemberInfo>> groups = new();
            public readonly List<MemberInfo> singleFields = new();
        }

        readonly Dictionary<Beautify.SectionGroup, SectionContents> sections = new();
        readonly Dictionary<Beautify.SettingsGroup, List<MemberInfo>> groupedFields = new();
        readonly Dictionary<MemberInfo, SerializedDataParameter> unpackedFields = new();
        ...
        public override void OnEnable()
        {
            ...
            IEnumerable<FieldInfo> settings = beautify.GetType()
            ...
            foreach (FieldInfo setting in settings)
            {
                ...
                foreach (Beautify.SectionGroup section in setting.GetCustomAttributes(typeof(Beautify.SectionGroup)) as
                             IEnumerable<Beautify.SectionGroup>)
                {
                    ...
                    foreach (Beautify.SettingsGroup settingGroup in setting.GetCustomAttributes(
                                 typeof(Beautify.SettingsGroup)) as IEnumerable<Beautify.SettingsGroup>)
                    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            int prevBloomExclusionLayerMask = beautify.bloomExclusionLayerMask.overrideState
                ? beautify.bloomExclusionLayerMask.value
                : 0;
            int prevAnamorphicFlaresExclusionLayerMask = beautify.anamorphicFlaresExclusionLayerMask.overrideState
            ...
            EditorGUILayout.BeginVertical();
            {
                ...
                if (GUILayout.Button(new GUIContent("Clear Effects", "Clears all effect overrides"),
                        EditorStyles.miniButton))
                {
                    if (EditorUtility.DisplayDialog("Clear Effects", "Do you want to clear all effects?", "Yes",
                            "Cancel"))
                    {
                ...
                if (GUILayout.Button(
                        new GUIContent("Quick Settings",
                            "Applies a default set of effects including color improvement, sharpening, vignette and bloom."),
                        EditorStyles.miniButton))
                {
                    if (EditorUtility.DisplayDialog("Quick Settings",
                            "Do you want to apply a collection of example effect settings (including color improvements, dithering, sharpening, vignette and bloom)?\nYou can adjust them later as you wish.",
                            "Yes", "No"))
                    {
                ...
                if (cam != null)
                {
                    ...
                    if (data != null && !data.renderPostProcessing &&
                        !BeautifyRendererFeature.ignoringPostProcessingOption)
                    {
                        EditorGUILayout.HelpBox(
                            "Post Processing option is disabled in the camera. Either enable it or enable the option 'Ignore Post Processing Option' in the Beautify Render Feature.",
                            MessageType.Warning);
                        EditorGUILayout.BeginHorizontal();
                ...
                {
                    EditorGUILayout.HelpBox(
                        "Universal Rendering Pipeline asset is not set in 'Project Settings / Graphics' !",
                        MessageType.Error);
                    EditorGUILayout.Separator();
                    GUI.enabled = false;
                }
                ...
                {
                    EditorGUILayout.HelpBox("Beautify Render Feature must be added to the rendering pipeline renderer.",
                        MessageType.Error);
                    if (GUILayout.Button("Go to Universal Rendering Pipeline Asset"))
                    {
                        Selection.activeObject = pipe;
                ...
                if (usesHDREffect && (QualitySettings.activeColorSpace != ColorSpace.Linear ||
                                      (Camera.main != null && !Camera.main.allowHDR)))
                {
                    EditorGUILayout.HelpBox(
                        "Some effects, like bloom or tonemapping, works better with Linear Color Space and HDR enabled. Enable Linear color space in Player Settings and check your camera and pipeline HDR setting.",
                        MessageType.Warning);
                }
                ...
                // sections
                foreach (KeyValuePair<Beautify.SectionGroup, SectionContents> section in sections)
                {
                    bool printSectionHeader = true;

                    // individual properties
                    foreach (MemberInfo field in section.Value.singleFields)
                    {
                        if (!unpackedFields.TryGetValue(field, out SerializedDataParameter parameter))
                        {
                            continue;
                        }
                        ...
                        if (printSectionHeader)
                        {
                            ...
                            GUI.Box(rect, ObjectNames.NicifyVariableName(section.Key.GetType().Name),
                                sectionGroupStyle);
                        ...
                        if (field.GetCustomAttribute(typeof(Beautify.DisplayName)) is Beautify.DisplayName
                            displayNameAttrib)
                        {
                    ...
                    // grouped properties
                    foreach (KeyValuePair<Beautify.SettingsGroup, List<MemberInfo>> group in section.Value.groups)
                    {
                        ...
                        foreach (MemberInfo field in group.Value)
                        {
                            if (!unpackedFields.TryGetValue(field, out SerializedDataParameter parameter))
                            {
                                continue;
                            ...
                            if (printGroupFoldout)
                            {
                                ...
                                settingsGroup.IsExpanded = EditorGUILayout.Foldout(settingsGroup.IsExpanded, groupName,
                                    true, foldoutStyle);
                            ...
                            if (parameter.value.propertyType == SerializedPropertyType.Boolean)
                            {
                                ...
                                {
                                    bool hasToggleSectionBegin =
                                        field.GetCustomAttributes(typeof(Beautify.ToggleAllFields)).Any();
                            ...
                            else if (field.Name.Equals("depthOfFieldFocusMode"))
                            {
                                if (BeautifySettings.instance != null &&
                                    BeautifySettings.instance.depthOfFieldTarget == null)
                                {
                                    SerializedProperty prop = serializedObject.FindProperty(field.Name);
                                    if (prop != null)
                                    {
                                        SerializedProperty value = prop.FindPropertyRelative("m_Value");
                                        if (value != null && value.enumValueIndex ==
                                            (int)Beautify.DoFFocusMode.FollowTarget)
                                        {
                                            EditorGUILayout.HelpBox("Assign target in the Beautify Settings component.",
                                                MessageType.Info);
                                        }
                                    }
                                }
            ...
            if (pixelateExpanded)
            {
                EditorGUILayout.HelpBox("Use the Downsampling option in General Settings to apply a pixelate effect.",
                    MessageType.Info);
            }
        ...
        bool IsVisible(SerializedDataParameter property, MemberInfo field, out bool indent)
        {
            ...
            Beautify.DisplayConditionEnum enumCondition =
                field.GetCustomAttribute(typeof(Beautify.DisplayConditionEnum)) as Beautify.DisplayConditionEnum;
            Beautify.DisplayConditionBool boolCondition =
                field.GetCustomAttribute(typeof(Beautify.DisplayConditionBool)) as Beautify.DisplayConditionBool;
            ...
            if (isEnumCondition)
            {
                ...
                if (condProp != null)
                {
                    SerializedProperty value = condProp.FindPropertyRelative("m_Value");
                    if (value != null)
                    {
                        visible = false;
            ...
            if (isBoolCondition)
            {
                ...
                if (condProp2 != null)
                {
                    SerializedProperty value2 = condProp2.FindPropertyRelative("m_Value");
                    if (value2 != null)
                    {
            ...
            if (property.value.propertyType == SerializedPropertyType.Boolean)
            {
                if (field.GetCustomAttribute(typeof(Beautify.GlobalOverride)) != null)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        Rect overrideRect = GUILayoutUtility.GetRect(w, 20f, GUILayout.ExpandWidth(false));
                        ...
                        if (value != prev)
                        {
                            ...
                            if (prop != null)
                            {
                                SerializedProperty boolProp = prop.FindPropertyRelative("m_Value");
                                if (boolProp != null)
                                {
                                ...
                                if (value)
                                {
                                    SerializedProperty overrideProp = prop.FindPropertyRelative("m_OverrideState");
        ...
        void SetStyles()
        {
            ...
            Color titleColor = EditorGUIUtility.isProSkin
            ...
            // foldout style
            Color foldoutColor = EditorGUIUtility.isProSkin
                ? new Color(0.52f, 0.66f, 0.9f)
                : new Color(0.12f, 0.16f, 0.4f);
        ...
        public class MaxFloatParameterDrawer : VolumeParameterDrawer
        {
            public override bool OnGUI(SerializedDataParameter parameter, GUIContent title)
            {
                ...
                {
                    Beautify.MinMaxFloatParameter o = parameter.GetObjectRef<Beautify.MinMaxFloatParameter>();
                    Vector2 range = o.value;
                    float x = range.x;
                    float y = range.y;
        ...
        Texture2D MakeTex(int width, int height, Color col)
        {
            ...
            TextureFormat tf = SystemInfo.SupportsTextureFormat(TextureFormat.RGBAFloat)
                ? TextureFormat.RGBAFloat
                : TextureFormat.RGBA32;
            Texture2D result = new(width, height, tf, false);
