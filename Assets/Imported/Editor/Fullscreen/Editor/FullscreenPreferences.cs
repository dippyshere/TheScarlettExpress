...
namespace FullscreenEditor
{
    ...
    public static class FullscreenPreferences
    {
        ...
        const string DEVELOPER_EMAIL = "samuelschultze@gmail.com";
        const string ASSET_STORE_PAGE = "https://assetstore.unity.com/packages/tools/utilities/fullscreen-editor-69534";
        const string CHANGE_LOG_LINK = ASSET_STORE_PAGE + "#releases";
        const string REVIEWS_LINK = ASSET_STORE_PAGE + "#reviews";
        const string FORUM_THREAD = "https://forum.unity.com/threads/released-fullscreen-editor.661519/";

        /// <summary>Current version of the Fullscreen Editor plugin.</summary>
        public static readonly Version pluginVersion = new(2, 2, 3);

        /// <summary>Release date of this version.</summary>
        public static readonly DateTime pluginDate = new(2021, 03, 20);

        static readonly GUIContent resetSettingsContent = new("Use Defaults", "Reset all settings to default ones");

        static readonly GUIContent versionContent =
            new(string.Format("Version: {0} ({1:d})", pluginVersion, pluginDate));

        static readonly GUIContent[] links =
        {
        ...
        static readonly string[] mosaicDropDownOptions =
        {
            "virtual display 1",
            "virtual display 2",
            "virtual display 3",
        ...
        static readonly PrefItem<Vector2> scroll = new("Scroll", Vector2.zero, string.Empty, string.Empty);
        ...
        {
            string rectSourceTooltip = string.Empty;
        ...
        {
            SettingsProvider sp = new("Preferences/Fullscreen Editor", SettingsScope.User,
        ...
        [SettingsProvider]
        static SettingsProvider RetrieveSettingsProviderShortcuts()
        {
            SettingsProvider sp = new("Preferences/Fullscreen Editor/Shortcuts", SettingsScope.User,
                contents.Select(c => c.text));
            sp.footerBarGuiHandler = OnFooterGUI;
            sp.guiHandler = search =>
            {
        ...
        static void OnPreferencesGUI(string search)
        {
            ...
            switch (RectSource.Value)
            {
                case RectSourceMode.Custom:
                    ...
                    Rect customRect = CustomRect.Value;
            ...
            if (FullscreenRects.ScreenCount > 1)
            {
                ...
                EditorGUI.indentLevel++;
                GUI.changed = false;
                int[] mosaicMapping = MosaicMapping.Value;

                for (int i = 0; i < mosaicMapping.Length && i < FullscreenRects.ScreenCount; i++)
                {
                    int val = EditorGUILayout.IntPopup(string.Format("Physical display {0} renders", i + 1),
        ...
        static void OnFooterGUI()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                {
                    GUI.changed = false;
                    bool enable = GUILayout.Toggle(Integration.IsDirectiveDefined("FULLSCREEN_DEBUG"), "Debug",
                        "Button");
                    if (GUI.changed)
                    {
        ...
        {
            StackFrame stack = new(0, true);
            string currentFile = stack.GetFileName();
            string currentPath = Path.GetDirectoryName(currentFile);

            return Path.Combine(currentPath, "../" + file);
        }

        static string GetEmailURL(Exception e = null)
        {
