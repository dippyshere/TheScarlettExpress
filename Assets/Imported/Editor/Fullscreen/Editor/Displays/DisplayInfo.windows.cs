...
namespace FullscreenEditor
{
    ...
    public class DisplayInfo
    {
        ...
        public Rect DpiCorrectedArea
        {
            get
            {
                DisplayInfo firstDisplayInfo = GetDisplay(0);
                Rect monitorArea = MonitorArea;

                Vector2 origin = monitorArea.min;
                Vector2 size = monitorArea.size;
        ...
        static void AddAdditionalInfos(List<DisplayInfo> displayInfo)
        {
        ...
        static void TransformToPixels(double unitX, double unitY, out int pixelX, out int pixelY)
        {
            IntPtr hDc = User32.GetDC(IntPtr.Zero);

            if (hDc != IntPtr.Zero)
            {
                int dpiX = GDI32.GetDeviceCaps(hDc, (int)GDI32.DeviceCap.VERTRES);
                int dpiY = GDI32.GetDeviceCaps(hDc, (int)GDI32.DeviceCap.DESKTOPVERTRES);
