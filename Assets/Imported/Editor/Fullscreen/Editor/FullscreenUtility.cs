...
namespace FullscreenEditor
{
    ...
    class SuppressLog : IDisposable
    {
        readonly bool lastState;
    ...
    public static class FullscreenUtility
    {
        ...
        static void RecalculateMousePosition()
        {
        ...
        internal static void ShowFullscreenExitNotification(EditorWindow window, string menuItemPath)
        {
            ...
            string notification = string.Format("Press {0} to exit fullscreen", TextifyMenuItemShortcut(menuItemPath));
        ...
        {
            int index = menuItemPath.LastIndexOf(" ");
            ...
            string shortcut = menuItemPath.Substring(index).Replace("_", "");
            Event evt = Event.KeyboardEvent(shortcut);
        ...
        static Texture2D FindOrLoadTexture(string base64)
        {
            Texture2D found = GetRef<Texture2D>(base64);

            return found ? found : LoadTexture(base64);
        }

        static Texture2D LoadTexture(string base64)
        {
            try
            {
                Texture2D texture = new(0, 0, TextureFormat.ARGB32, false, true);
        ...
        {
            UnityObject[] containers = Resources.FindObjectsOfTypeAll(Types.MainView);
        ...
        public static EditorWindow GetMainGameView()
        {
            ...
            if (Types.PreviewEditorWindow.HasMethod("GetMainPreviewWindow"))
            {
        ...
        {
            ScriptableObject mostSpecificView = Types.GUIView.GetPropertyValue<ScriptableObject>("focusedView");
            ...
            ScriptableObject[] viewHierarchy = GetViewHierarchy(mostSpecificView);
            ScriptableObject leastSpecificView = viewHierarchy.LastOrDefault();
            ...
            bool alone = leastSpecificView.GetPropertyValue<Array>("allChildren").Length == viewHierarchy.Length;
        ...
        public static ScriptableObject[] GetViewHierarchy(ScriptableObject view)
        {
            ...
            List<ScriptableObject> list = new() { view };
            ScriptableObject parent = view.GetPropertyValue<ScriptableObject>("parent");
        ...
        public static bool IsViewFocused(ScriptableObject view)
        {
            ...
            ScriptableObject focused = Types.GUIView.GetPropertyValue<ScriptableObject>("focusedView");
            ScriptableObject[] children = GetAllViewChildren(view);
        ...
        public static void FocusView(ScriptableObject guiView)
        {
            ...
            {
                ViewPyramid vp = new(guiView);
                ScriptableObject vc = vp.Container;
        ...
        public static float GetToolbarHeight()
        {
            try
            {
                ...
                {
                    object result = typeof(EditorGUI).GetFieldValue<object>("kWindowToolbarHeight");
                    if (result is int)
                    {
                        return (int)result;
                    }
